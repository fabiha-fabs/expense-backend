...
export class ExpenseService {
  ...
  async createExpense(
    userCreateRequest: ExpenseCreateRequest,
  ): Promise<Expense> {
  ...
  async findOne(expenseId: number): Promise<Expense> {
    const expenseFindById: Expense = await this.expenseRepository.findOne(
      expenseId,
    );
  ...
  async updateExpense(id: number, expenseUpdateRequest: ExpenseUpdateRequest) {
    await this.expenseRepository.update(id, expenseUpdateRequest);
    const updatedExpense: Expense = await this.expenseRepository.findOne(id);
    return plainToClass(ExpenseUpdateRequest, updatedExpense);
  }
